...
class MetricsCache(object):
	def __init__(self, hostmoid, vmmoid, vmmwl=[], vmmbl=[], hostmwl=[], hostmbl=[]):
		...
		self.vmmoid=vmmoid
		self.vmmwl = [re.compile(x) for x in vmmwl]
		self.vmmbl = [re.compile(x) for x in vmmbl]
		self.hostmwl = [re.compile(x) for x in hostmwl]
		self.hostmbl = [re.compile(x) for x in hostmbl]
	...
	def _pruneWhiteBlacklists(self, listtoprune, whitelist, blacklist):
		...
		for metric in listtoprune:
			...
			if whitelist and blacklist:
				...
				processmetric=False
				if [regexmatch for regexmatch in whitelist if regexmatch.match(metric['name'])] and not [regexmatch for regexmatch in blacklist if regexmatch.match(metric['name'])]:
					processmetric=True
			elif whitelist and not blacklist:
				#There is a white list and no blacklist
				processmetric=False
				if [regexmatch for regexmatch in whitelist if regexmatch.match(metric['name'])]:
					processmetric=True
			elif not whitelist and blacklist:
				#There is no whitelist and there is a blacklist
				if [regexmatch for regexmatch in blacklist if regexmatch.match(metric['name'])]:
					processmetric=False
			if processmetric:
				res.append(metric)
		return res
	...
	def _getCounterListsNames(self):
		...
		if self.hostmwl or self.hostmbl:
