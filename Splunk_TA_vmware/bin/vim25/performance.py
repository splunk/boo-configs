...
class PerfCollector(Connection):
	...
	def _prepare_metrics_lists(self, entities, vc_id):
		...
		metrics = {}
		entity_mbwl = {'clustermwl': self.config.get("cluster_metric_whitelist", []),
					   'clustermbl' : self.config.get("cluster_metric_blacklist", []),
					   'rpmwl' : self.config.get("rp_metric_whitelist", []),
					   'rpmbl' : self.config.get("rp_metric_blacklist", [])}

		if self.domain not in self._metrics_cache:
			self._metrics_cache[self.domain] = MetricsCache(2000, 50)
		if self.domain not in self._entity_mbwl_cache:
			self._entity_mbwl_cache[self.domain] = {}
		cache = self._metrics_cache[self.domain]
		if inventory_hash in cache and (entity_mbwl == self._entity_mbwl_cache[self.domain]):
			...
			self._entity_mbwl_cache[self.domain] = entity_mbwl
