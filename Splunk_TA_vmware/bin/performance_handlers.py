...
class HostVMPerfHandler(BasePerfHandler):
	...
	def run(self, session, config, create_time, last_time):
		...
		try:
			...
			entity_mbwl = {'vmmwl': config.get("vm_metric_whitelist", []),
						   'vmmbl' : config.get("vm_metric_blacklist", []),
						   'hostmwl' : config.get("host_metric_whitelist", []),
						   'hostmbl' : config.get("host_metric_blacklist", [])}
			...
			if self.metricscachedict:
				...
				self.entitymbwlcache = self.metricscachedict.get('entity_mbwl_cache', {})
				if not entity_mbwl == self.entitymbwlcache:
			...
			if not self.metricscache:
				...
				if not self.vms_on_host:
					...
					self.metricscache = hostvm_metrics.MetricsCache(hostmoid=config["perf_target_hosts"][0], vmmoid=[], hostmwl=entity_mbwl['hostmwl'], hostmbl=entity_mbwl['hostmbl']).fullcounters
				else:
					self.metricscache = hostvm_metrics.MetricsCache(hostmoid=config["perf_target_hosts"][0], vmmoid=self.vms_on_host[config["perf_target_hosts"][0]], vmmwl=entity_mbwl['vmmwl'], vmmbl=entity_mbwl['vmmbl'], hostmwl=entity_mbwl['hostmwl'], hostmbl=entity_mbwl['hostmbl']).fullcounters
				if self.metricscache:
					metric_dict = {"metric_cache" : self.metricscache, "entity_mbwl_cache" : entity_mbwl}
			...
			for host_moid in config['perf_target_hosts']:
				if not self.pc._is_entity_blacklisted('HostSystem') and self.metricscache['hostmetrics']:
				...
				if self.vms_on_host:
					for vm_moid in self.vms_on_host[host_moid]:
						if not self.pc._is_entity_blacklisted('VirtualMachine') and self.metricscache['vmmetrics']:
							#runs through the metric cache and builds metric objects based on the correct instance.
							metrics=[]
